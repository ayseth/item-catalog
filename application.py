...
def showitems(catalog_category):
	...
	if 'username' not in login_session :
		return render_template('publicshowitems.html', listcatalogs = listcatalogs, count=count, title=catalog_category)
	else:
...
def newitem():
	...
	if request.method == 'POST':
		newItem = catalog(title=request.form['title'], description=request.form['description'], category=request.form['category'], picture=request.form['picture'], link=request.form['link'], cover_pic=request.form['cover_pic'],  user_id=login_session['user_id'])
		session.add(newItem)
		session.commit()
...
def showitem(catalog_category, catalog_title):
	displaycontent=session.query(catalog).filter_by(title=catalog_title).one()
	creator = getUserInfo(displaycontent.user_id)
	if 'username' not in login_session or creator.id != login_session['user_id']:
		return render_template('publicshowitem.html',displaycontent=displaycontent, creator=creator)
	else:
		return render_template('showitem.html', displaycontent = displaycontent, creator=creator)
...
def edititem(catalog_title):
	...
	if editItem.user_id != login_session['user_id']:
		return "<script>function myFunction() {alert('You are not authorized to edit this. Please create your own item in order to edit.');}</script><body onload='myFunction()''>"
	if request.method == 'POST':
